...
if ($_REQUEST['act']== 'list')
{
    ...
    assign_query_info();
    $flash_dir = ROOT_PATH . 'data/flasblockTitleata/';
...
elseif($_REQUEST['act']== 'del')
{
    ...
    $id = (int)$_GET['id'];
    $flasblockTitleb = get_flash_xml();
    if (isset($flasblockTitleb[$id]))
    {
        $rt = $flasblockTitleb[$id];
    }
    else
    {
    ...
    $temp = array();
    foreach ($flasblockTitleb as $key => $val)
    {
        if ($key != $id)
        {
            $temp[] = $val;
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    elseif ($_POST['step'] == 2)
    {
        ...
        // 插入新数据
        array_unshift($flasblockTitleb, array('src'=>$src, 'url'=>$_POST['img_url'], 'text'=>$_POST['img_text'] ,'sort'=>$_POST['img_sort']));

        // 实现排序
        $flasblockTitleb_sort   = array();
        $_flasblockTitleb       = array();
        foreach ($flasblockTitleb as $key => $value)
        {
            $flasblockTitleb_sort[$key] = $value['sort'];
        }
        asort($flasblockTitleb_sort, SORT_NUMERIC);
        foreach ($flasblockTitleb_sort as $key => $value)
        {
            $_flasblockTitleb[] = $flasblockTitleb[$key];
        }
        unset($flasblockTitleb, $flasblockTitleb_sort);

        put_flash_xml($_flasblockTitleb);
        set_flash_data($_CFG['flash_theme'], $error_msg = '');
        $links[] = array('text' => $_LANG['go_url'], 'href' => 'flashplay.php?act=list');
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $flasblockTitleb = get_flash_xml(); //取得数据
    if (isset($flasblockTitleb[$id]))
    {
        $rt = $flasblockTitleb[$id];
    }
    ...
    elseif ($_POST['step'] == 2)
    {
        ...
        $flasblockTitleb[$id] = array('src'=>$src,'url'=>$_POST['img_url'],'text'=>$_POST['img_text'],'sort'=>$_POST['img_sort']);
    ...
    if (file_exists(ROOT_PATH . DATA_DIR . '/flash_data.xml'))
    {
        ...
        if (!empty($t))
        {
            foreach ($t as $key => $val)
            {
                ...
                $flasblockTitleb[] = array('src'=>$val[1],'url'=>$val[2],'text'=>$val[3],'sort'=>$val[4]);
            }
        }
    }
    return $flasblockTitleb;
}

function put_flash_xml($flasblockTitleb)
{
    if (!empty($flasblockTitleb))
    {
...
function get_flash_tpl_info($dir, $file)
{
    ...
    {
        $info['code'] = $file;
        $info['screenshot'] = '../data/flasblockTitleata/' . $file . '/preview.jpg';
...
{
    $flasblockTitleata = get_flash_xml();
    if (empty($flasblockTitleata))
    {
        $flasblockTitleata[] = array(
    ...
    {
        case 'uproll':
            $msg = set_flash_uproll($tplname, $flasblockTitleata);
            break;
        case 'redfocus':
        case 'pinkfocus':
        case 'dynfocus':
            $msg = set_flash_focus($tplname, $flasblockTitleata);
            break;
        case 'default':
        default:
            $msg = set_flash_default($tplname, $flasblockTitleata);
            break;
    }
    return $msg !== true;
}

function set_flash_uproll($tplname, $flasblockTitleata)
{
    $data_file = ROOT_PATH . DATA_DIR . '/flasblockTitleata/' . $tplname . '/data.xml';
    $xmldata = '<?xml version="1.0" encoding="' . EC_CHARSET . '"?><myMenu>';
    foreach ($flasblockTitleata as $data)
    {
...
function set_flash_focus($tplname, $flasblockTitleata)
{
    $data_file = ROOT_PATH . DATA_DIR . '/flasblockTitleata/' . $tplname . '/data.js';
    $jsdata = '';
...
function set_flash_default($tplname, $flasblockTitleata)
{
    $data_file = ROOT_PATH . DATA_DIR . '/flasblockTitleata/' . $tplname . '/data.xml';
