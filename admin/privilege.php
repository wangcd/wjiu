...
elseif ($_REQUEST['act'] == 'set_cat')
{
    /* 检查权限 */
    admin_priv('setcatlist');
	$cats_id = !empty($_POST['checkboxes']) ? join(',', $_POST['checkboxes']) : 0;
	$id = !empty($_REQUEST['id']) ? intval($_REQUEST['id']) : 0;
	clear_cache_files();
	
	$cat_arr = array();
	foreach(explode(',',$cats_id) as $k)
	{
		$cat_arr = array_unique(array_merge($cat_arr, array_keys(cat_list($k,0,false))));
	}
	$cat_arr = implode(',', $cat_arr);
    $GLOBALS['db']->query("UPDATE " . $GLOBALS['ecs']->table('admin_user') . " SET cat_list = '$cat_arr' where user_id = '$id' ");

	/* 提示信息 */
	$link[] = array('text' => '返回', 'href'=>'privilege.php?act=catlist&id=$id');
	sys_msg('授权成功', 0, $link);

	exit;	
}
elseif ($_REQUEST['act'] == 'catlist')
{
    /* 检查权限 */
    admin_priv('setcatlist');
    $id = !empty($_REQUEST['id']) ? intval($_REQUEST['id']) : 0;
    ...
    $list = $db->getOne("SELECT cat_list FROM " . $ecs->table("admin_user") . " where user_id = '$id' ");
	$res = explode(',',$list);
    ...
    if (!empty($res))
    {
        foreach($res as $data)
        {
			if($data)$cat_list[$data]['sq'] = 1;
        }
    }

    /* 模板赋值 */
    $smarty->assign('ur_here',      '授权分类管理商品');
    $smarty->assign('full_page',    1);
    $smarty->assign('cat_info',     $cat_list);

    /* 显示页面 */
    assign_query_info();
    $smarty->display('setcatlist.htm');
}
