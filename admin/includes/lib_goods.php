...
function get_attr_list($cat_id, $goods_id = 0)
{
    ...
    // 查询属性值及商品的属性值
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_input_type, a.attr_type, a.attr_values, v.attr_order, v.attr_value, v.attr_price ".
            "FROM " .$GLOBALS['ecs']->table('attribute'). " AS a ".
            "LEFT JOIN " .$GLOBALS['ecs']->table('goods_attr'). " AS v ".
            "ON v.attr_id = a.attr_id AND v.goods_id = '$goods_id' ".
            "WHERE a.cat_id = " . intval($cat_id) ." OR a.cat_id = 0 ".
            "ORDER BY a.sort_order, a.attr_type, a.attr_id, v.attr_price, v.goods_attr_id";  //by Leah
    ...
    foreach ($attr AS $key => $val)
    {
       ...
       $html .= ($val['attr_type'] == 1 || $val['attr_type'] == 2) ?
            $GLOBALS['_LANG']['spec_price'].' <input type="text" name="attr_price_list[]" value="' . $val['attr_price'] . '" size="5" maxlength="10" />' 
                .$GLOBALS['_LANG']['attr_order'] .'<input type="text" name="attr_order_list[]" value="' . $val['attr_order'] . '" size="5" maxlength="10" />':
            ' <input type="hidden" name="attr_price_list[]" value="0" /> <input type="hidden" name="attr_order_list[]" value="0" />';  // by leah
...
//授权分类管理商品_新增加 开始 by www.edait.cn
function sq_list($catid)
{
	$txt = '';
	$sqcats = $GLOBALS['db']->getOne("SELECT cat_list FROM " . $GLOBALS['ecs']->table("admin_user") . " where user_id = '$_SESSION[admin_id]' ");
	if(!empty($sqcats)){
	$sqcats = $GLOBALS['db']->getAll("SELECT cat_id,cat_name FROM " . $GLOBALS['ecs']->table("category") . " WHERE cat_id " . db_create_in($sqcats));
	foreach($sqcats as $v)
	{
		if($catid == $v[cat_id] )
		{
		$txt .= '<option value="'.$v[cat_id].'" selected="selected" >'.$v[cat_name].'</option>';
		
		break;
		}
		else{
		$txt .= '<option value="'.$v[cat_id].'" >'.$v[cat_name].'</option>';
		}
	}	
	return $txt;
	}else{
	return cat_list(0, $cat_id);
	}
}
//授权分类管理商品_新增加 结束 by www.edait.cn
...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        $sqcats = $GLOBALS['db']->getOne("SELECT cat_list FROM " . $GLOBALS['ecs']->table("admin_user") . " where user_id = '$_SESSION[admin_id]' ");
        if(!empty($sqcats)){
            $where .= " AND cat_id " . db_create_in($sqcats);
        }
        //授权分类管理商品_新增加 结束 by www.edait.cn
