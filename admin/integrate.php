...
if ($_REQUEST['act'] == 'task')
{
    ...
    else if ($_SESSION['task']['sync']['start'] < $_SESSION['task']['sync']['total'])
    {
        ...
        $sql = "SELECT user_name, password, email, sex, birtblockTitleay, reg_time ".
                "FROM " . $ecs->table('users') . " LIMIT " . $_SESSION['task']['del']['start'] . ',' . $result['size'];
        $arr = $db->getAll($sql);
        foreach ($arr as $user)
        {
            @$cls_user->add_user($user['user_name'], '', $user['email'], $user['sex'], $user['birtblockTitleay'], $user['reg_time'], $user['password']);
        }
