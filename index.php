...
function index_get_group_buy()
{
	...
	if ($limit >0)
	{
		$sql = 'SELECT gb.act_id AS group_buy_id, gb.goods_id, gb.ext_info, gb.goods_name, gb.act_desc, gb.end_time AS end_date , gb.start_time AS start_date ,g.goods_thumb, g.goods_img, g.market_price,g.shop_price, g.click_count '.
		...
		$row['gmt_end_date'] = local_date('M d, Y H:i:s',$row['end_date']);
		...
		else
		{
			...
			$row['order_number'] = $GLOBALS['db']->GetOne($sql);
			ksort($price_ladder);
			$row['last_price'] = (end($price_ladder));
			$row['market_price'] = price_format($row['market_price']);
			$row['jiesheng'] = $row['shop_price']-(end($price_ladder));
