...
function insert_history()
{
    ...
    if (!empty($_COOKIE['ECS']['history']))
    {
        ...
        while ($row = $GLOBALS['db']->fetch_array($query))
        {
			...
			<li class="goods_text"><a  class="name" href="'.$goods['url'].'" target="_blank" title="'.$goods['goods_name'].'">'.$goods['short_name'].'</a>
			
			'.'<div class="price">'.$goods['shop_price'].' 
			</div>
			<br />
			</li>
			</ul>';
        }
        $str .= '<span id="clear_history"><a onclick="clear_history()">' . $GLOBALS['_LANG']['clear_history'] . '</a></span>';
    }
    return $str;
}
...
function insert_cart_info_two()
{
    ...
    $output = $GLOBALS['smarty']->fetch('library/cart_info_two.lbi');
    return $output;
}
