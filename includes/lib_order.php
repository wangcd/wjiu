...
function get_give_integral()
{
	  ...
	  $sql = "SELECT SUM(c.goods_number * IF(g.integral_putong > -1, g.integral_putong, c.goods_price))" .
                "FROM " . $GLOBALS['ecs']->table('cart') . " AS c, " .
                          $GLOBALS['ecs']->table('goods') . " AS g " .
                "WHERE c.goods_id = g.goods_id " .
                "AND c.session_id = '" . SESS_ID . "' " .
...
function integral_to_give($order)
{
    ...
    else
    {
	   ...
	   $sql = "SELECT SUM(og.goods_number * IF(g.integral_putong > -1, g.integral_putong, og.goods_price)) AS custom_points, SUM(og.goods_number * IF(g.rank_integral > -1, g.rank_integral, og.goods_price)) AS rank_points " .
