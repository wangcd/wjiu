...
{
	$sql = 'SELECT sum(goods_number) FROM '.$GLOBALS['ecs']->table('order_goods')." WHERE goods_id = '$goods_id' GROUP BY goods_id";
 ...
 * return $res
 */
function get_new_comment1($goods_id,$limit = 6)
{
	$sql = 'SELECT user_name,comment_rank,content FROM '.$GLOBALS['ecs']->table('comment')." WHERE id_value='$goods_id' AND comment_type=0 AND status = 1 ORDER BY comment_id DESC LIMIT ".$limit;
	$res = $GLOBALS['db']->GetAll($sql);
	foreach($res as $key=>$val)
	{
		if(!empty($val['comment_rank']))
		{
			$res[$key]['comment_rank'] = $val['comment_rank']*20;
		}
	}
	return $res;
 ...
 * return $row
 */
function get_ratenum($goods_id)
{
 	$sql = 'SELECT sum(comment_rank) AS rank_sum,count(*) AS number FROM '.$GLOBALS['ecs']->table('comment')." WHERE id_value='$goods_id' AND status !=0 GROUP BY id_value "; 
	$row = $GLOBALS['db']->getRow($sql);

	if(!empty($row['number']))
	{
		$row['ratenum'] = sprintf("%1\$.0f",20*($row['rank_sum']/$row['number']));
	}
	else
	{
		$row['ratenum'] = 0;
	...
	$sql = 'SELECT comment_rank,count(comment_rank) AS sum FROM '.$GLOBALS['ecs']->table('comment')." WHERE id_value='$goods_id' GROUP BY comment_rank ";
	$rank_res = $GLOBALS['db']->getAll($sql);
	
	foreach($rank_res as $key=>$value)
	{			
		$arr[$value['comment_rank']]['comment_rank'] = $value['comment_rank'];
		$arr[$value['comment_rank']]['sum']          = $value['sum'];
	
	}
	if(!empty($arr[1]))
	{
		$row['rank_1'] = $arr[1]['sum'];
		$row['bfb_1'] = sprintf("%1\$.0f",(($arr[1]['sum']*1)/$row['rank_sum'])*100);	
	}
	else
	{
		$row['rank_1'] = 0;
	...
	if(!empty($arr[2]))
	{
		$row['rank_2'] = $arr[2]['sum'];
		$row['bfb_2'] = sprintf("%1\$.0f",(($arr[2]['sum']*1)/$row['rank_sum'])*100);	
	}
	else
	{
		$row['rank_2'] = 0;
	...
	if(!empty($arr[3]))
	{
		$row['rank_3'] = $arr[3]['sum'];
		$row['bfb_3'] = sprintf("%1\$.0f",(($arr[3]['sum']*1)/$row['rank_sum'])*100);	
	}
	else
	{
		$row['rank_3'] = 0;
	...
	if(!empty($arr[4]))
	{
		$row['rank_4'] = $arr[4]['sum'];
		$row['bfb_4'] = sprintf("%1\$.0f",(($arr[4]['sum']*1)/$row['rank_sum'])*100);	
	}
	else
	{
		$row['rank_4'] = 0;
	...
	if(!empty($arr[5]))
	{
		$row['rank_5'] = $arr[5]['sum'];
		$row['bfb_5'] = sprintf("%1\$.0f",(($arr[5]['sum']*1)/$row['rank_sum'])*100);	
	}
	else
	{
		$row['rank_5'] = 0;
 ...
 * @return  array
 */
function get_category_recommend_goods($type = '', $cats = '', $brand = 0, $min =0,  $max = 0, $ext='' , $limit='')
{
...
function get_goods_info($goods_id)
{
    ...
    if ($row !== false)
    {
       ...
       $row['promote_sheng_price'] = $row['market_price']-$row['promote_price'];
       $row['promote_zhekou_price'] = round(($row['promote_price']/$row['market_price'])*100)/10+"";
...
function get_goods_properties($goods_id)
{
    ...
    /* 获得商品的规格 */
      $sql = "SELECT a.attr_id, a.attr_name, a.attr_group, a.is_linked, a.attr_type, ".
            ...
            'ORDER BY a.sort_order, g.attr_price, g.goods_attr_id'; //// by leah
...
/* zhong改上 */
	...
	/* 销售排行 */
	$sql = 'SELECT g.goods_id,g.goods_name,sum(og.goods_number) AS number,g.shop_price,g.goods_thumb FROM '.$GLOBALS['ecs']->table('goods').' AS g LEFT JOIN '.$GLOBALS['ecs']->table('order_goods').' AS og ON g.goods_id=og.goods_id WHERE g.is_delete = 0 AND (' . $children . 'OR ' . get_extension_goods($children) . ') GROUP BY g.goods_id ORDER BY number DESC LIMIT 6';
	...
	{
		$sales_volume[$key]['url']          = build_uri('goods', array('gid' => $value['goods_id']), $value['goods_name']);
		$sales_volume[$key]['review_count'] = $GLOBALS['db']->getOne('select count(*) from ' . $GLOBALS['ecs']->table('comment') ." where status!=0 and id_value ='$value[goods_id]' ");
	}
	/* 评论数量 */
	...
	*/
	$sql = 'SELECT cat_id,cat_name FROM ' . $GLOBALS['ecs']->table('category') . " WHERE parent_id = '$cat_id' order by sort_order  ";
