...
$sort  = (isset($_REQUEST['sort'])  && in_array(trim(strtolower($_REQUEST['sort'])), array( 'shop_price', 'last_update',  'goods_id'))) ? trim($_REQUEST['sort'])  : $default_sort_order_type;
...
//zhong 改结束
        ...
        $arr[$row['goods_id']]['url']              = build_uri('goods', array('gid'=>$row['goods_id']), $row['goods_name']);
		 ...
		 if($row['market_price'] != 0)//市场价格不能为0，0不能为除数
  {
   if($row['promote_price'] == 0)//有促销价格的时候，用促销价格计算
   {
    $arr[$row['goods_id']]['zhekou_price'] = round(($row['shop_price']/$row['market_price'])*100)/100+"";
   }
   else  //没有促销价格即用本店价格计算
   {
    $arr[$row['goods_id']]['zhekou_price'] = round(($row['promote_price']/$row['market_price'])*100)/100+"";
   }
  }
  else
  {
   $arr[$row['goods_id']]['zhekou_price'] = 0;
  }
    }
